...
function Users() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Actions",
      render: (user) => {
        // Prevent admin from editing or deleting themselves
        if (session?.user?.id === user.id) {
          return <span className="font-semibold text-slate-500">Locked (Current User)</span>;
        }

        // Lock actions for ADMIN role
        if (user.role === 'ADMIN') {
